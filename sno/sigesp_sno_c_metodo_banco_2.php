...
class sigesp_sno_c_metodo_banco_2
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_limpiar_cadena($ls_cadena)
	{ 
		$ls_cadena=strtoupper($ls_cadena);
		$search  = array('ñ','á','é', 'í', 'ó', 'ú','Ñ','Á','É','Í','Ó','Ú');
		$replace = array('N','A','E', 'I', 'O', 'U','N','A','E', 'I', 'O', 'U');
		$ls_cadena2=str_replace("Ñ","N",$ls_cadena);		
		$ls_cadena2=str_replace($search,$replace,$ls_cadena2);		
		return $ls_cadena2;			
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	{ 
		...
		if ($li_count>0)
		{
			...
			{
				//Registro Cabecera (D�bito)
				$ls_nombre=$this->io_funciones->uf_rellenar_der(substr($this->ls_siglas,0,40)," ",40);
			...
			while((!$rs_data->EOF)&&($lb_valido))
			{
				...
				$ls_nomper=$this->uf_limpiar_cadena(trim($rs_data->fields["nomper"]));
				$ls_apeper=$this->uf_limpiar_cadena(trim($rs_data->fields["apeper"]));
				$ls_personal="".$this->io_funciones->uf_rellenar_der(substr($ls_apeper." ".$ls_nomper,0,40)," ",40);
				$ls_personal2=$this->uf_limpiar_cadena(utf8_encode($ls_personal));
				$ls_codtipcueban="";
				...
				$ls_cadena=$ls_tipcuebanper.$ls_codcueban.$ldec_neto.$ls_codtipcueban.$ls_personal2.$ls_cedper."003291  "."\r\n";
				if ($ls_creararchivo)
				{
					if (fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
					{
