<?php
	...
	function uf_print_encabezado_pagina1($as_titulo,$as_desnom,$as_periodo,$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_firma"],140,75,300,150); // Agregar firma
	...
	function uf_print_pie_cabecera1($ai_toting,$ai_totded,$ai_totnet,$as_codcueban,$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->addText(40,450,'10','______________________________________________________________________________________________');
		$io_pdf->ezSety(447);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('denomasig'=>array('justification'=>'center','width'=>150), // Justificación y ancho de la columna
						 			   ...
						 			   'valordedu'=>array('justification'=>'right','width'=>100))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(40,430,'10','______________________________________________________________________________________________');
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$io_pdf->addText(40,412,'10','______________________________________________________________________________________________');
		/*if($ls_tiporeporte==0)
		{
	...
	switch($ls_tiporeporte)
	{
		case "0":
			...
			$io_report=new sigesp_snorh_class_report($ls_anocurso);
			$ls_bolivares ="Bs.";
			break;
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$lb_valido=$io_report->uf_recibopago_conceptopersonal($ls_codnom,$ld_fecdesper,$ld_fechasper,$ls_codper,
					   ...
					   $li_texto=$io_pdf->addText(50,340,8,"S.S.O.      = SEGURO SOCIAL OBLIGATORIO");
					   $li_texto=$io_pdf->addText(50,330,8,"R.P.E.      = REGIMEN PRESTACIONAL DE EMPLEO");
					   $li_texto=$io_pdf->addText(50,320,8,"F.A.O.V.   = FONDO DE AHORRO OBLIGATORIO DE VIVIENDA");					   
                       $li_texto=$io_pdf->addText(50,310,8,"F.P.J.       = FONDO DE PENSIONES Y JUBILACIONES");
                       $li_texto=$io_pdf->addText(50,280,9,"Cualquier información adicional solicitarla a traves del correo: talentohumano@gmsaberytrabajo.gob.ve");
                       /*$li_texto=$io_pdf->addText(270,150,9,"<b>Maritza Casanova</b>");
                       $li_texto=$io_pdf->addText(225,140,9,"Directora de la Oficina de Gestion Humana");
                       $li_texto=$io_pdf->addText(230,130,9,"<b>Fundacion Gran Mision Saber y Trabajo</b>");*/
