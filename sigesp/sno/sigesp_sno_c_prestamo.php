			...
			if($lb_valido)
			{
				...
				else
				{
					...
					{	
						...
						if($this->mostrar_mensaje){$this->io_mensajes->message("EL prestamo fue Eliminado.");}	
						$this->io_sql->commit();
					}
					else
					{
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_validar_prestamo($parametros=array())
    {
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_validar_prestamo 
		//	      Returns: lb_existe True si existe ó False si no existe
		//	  Description: Funcion que valida que no se cargue repetido un prestamo.
		//	   Creado Por: Lic. Edgar A. Quintero U.
		// Fecha Creación: 16/10/2009								´
		...
		//////////////////////////////////////////////////////////////////////////////
 		$lb_existe=false;
       	...
       	$ls_sql="SELECT * ".
				...
				" WHERE codemp='".$this->ls_codemp."'".
				"   AND codnom='".$this->ls_codnom."'".
				"   AND codper='".$parametros['codper']."'".
				"   AND codtippre='".$parametros['codtippre']."'".
				"   AND codconc='".$parametros['codconc']."'".
				//"   AND monpre='".$parametros['monpre']."'".
				"   AND perinipre='".$parametros['perinipre']."'".
				"   AND fecpre='".$parametros['fecpre']."' ORDER BY numpre DESC";
		
       	$this->rs_data=$this->io_sql->select($ls_sql);
		//echo $ls_sql.'<br>';
       	if ($this->rs_data===false)
       	{
        	$this->io_mensajes->message("CLASE->Prestamo MÉTODO->uf_validar_prestamo ERROR->".$this->io_sql->message);
			return false;
       	}       	
			
		return 	$this->rs_data->RecordCount();
		    
	}// end function uf_select_prestamo

	function uf_validar_prestamos_iguales($parametros=array())
    {
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_validar_prestamo 
		//	      Returns: lb_existe True si existe ó False si no existe
		...
		//////////////////////////////////////////////////////////////////////////////
 		$lb_existe=false;
       	...
       	$ls_sql="   SELECT * 
					FROM sno_prestamosperiodo pp
					INNER JOIN sno_prestamos p ON p.codemp = pp.codemp
								   AND p.codnom = pp.codnom
								   AND p.codtippre = pp.codtippre
				    WHERE pp.codnom='".$this->ls_codnom."'
				    AND pp.codper='".$parametros['codper']."'
				    AND pp.codtippre='".$parametros['codtippre']."'				
				    AND pp.feciniper<='".$parametros['fecpre']."'
				    AND pp.fecfinper>='".$parametros['fecpre']."'
					AND p.fecpre<'".$parametros['fecpre']."'
					";
