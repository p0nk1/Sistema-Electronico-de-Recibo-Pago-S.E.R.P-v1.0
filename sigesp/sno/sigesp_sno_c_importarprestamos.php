	...
	function uf_load_personalprestamo($ai_nrofilas,$ai_totrow,$aa_seguridad)
	{
		...
		if($no_existe>0 or $errores>0){
				...
				$mensaje=$mensaje."' + ".'"\n"'." + ".'"\n"'.
						  ...
						  '"\n"'.
						  " + '*** ".$errores." personas con error en actualización. *** ' + ".
						  '"\n"'.
						  " + '*** Consulte el log de errores para verificar las cédulas. ***' + ".
						  '"\n"'.
						  " + ".'"\n"'.
				...
				$texto2 = 'Cédulas no encontradas: '."\r\n".$codigos_per."\r\n";
				$texto3 = 'Cédulas con error en actualización: '."\r\n".$codigos_act."\r\n";
...
function uf_buscar_concepto($as_codconc)
{
	...
	function uf_procesar_prestamo($datos,$aa_seguridad)
	{
		...
		if($existex){
				...
				$ls_cadena = ' *** ERROR: Ya tiene cuotas de un prestamo con este concepto en el período actual. No será actualizado. codper: '.$datos['codper'].' codconc: '.$datos['codconc'].
		...
		if($datos['eliminar']=='si' and $existe<1){							
				$ls_cadena = ' *** ERROR: El prestamo no existe para esta persona. No será eliminado. codper: '.$datos['codper'].' codconc: '.$datos['codconc'].
							 '    fecha: '.$datos['fecpre'].'    monto: '.$datos['monpre'];
				$this->logs->sislog($ls_cadena,$datos['nombrearchivo']);
				return false;							
		}
		...
		{
 			$lb_valido=false;
			$texto = "CLASE->sigesp_sno_c_importarprestamos MÉTODO->uf_procesar_prestamo ERROR->".$this->io_sql->message; 
					...
					if($existe>=1){					   
						while($existe>0){
							...
							if($this->io_prestamo->cuotascancel){
										$ls_cadena = 'No se puede eliminar el prestamo. Ya existen cuotas canceladas codper: '.$datos['codper'].'   codconc: '.$datos['codconc'].'   número: '.$this->io_prestamo->rs_data->fields['numpre'];
										$this->logs->sislog($ls_cadena,$datos['nombrearchivo']);
							...
							if(!$valido){
									...
									$ls_cadena = 'Error eliminando el prestamo codper: '.$datos['codper'].'   codconc: '.$datos['codconc'].'   número: '.$this->io_prestamo->rs_data->fields['numpre'];
									$this->logs->sislog($ls_cadena,$datos['nombrearchivo']);
									return false;
							
							}								
							$ls_cadena = 'El prestamo fué eliminado codper: '.$datos['codper'].'   codconc: '.$datos['codconc'].'   número: '.$this->io_prestamo->rs_data->fields['numpre'];
							$this->logs->sislog($ls_cadena,$datos['nombrearchivo']);
